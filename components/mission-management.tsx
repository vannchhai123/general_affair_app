...
export function MissionManagement() {
  ...
  function handleSave() {
    ...
    if (editingMission) {
      setMissions((prev) => prev.map((m) => (m.id === editingMission.id ? { ...m, ...form } : m)));
  ...
  function handleReject(mission: Mission) {
    setMissions((prev) =>
      prev.map((m) => (m.id === mission.id ? { ...m, status: 'Rejected' as const } : m)),
    );
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Create, approve, and track officer missions
          </p>
        </div>
          ...
          <DialogContent className="bg-card border-border text-card-foreground sm:max-w-lg">
            ...
            <div className="grid grid-cols-2 gap-4 py-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Select
                  value={form.status}
                  onValueChange={(v) => setForm({ ...form, status: v as Mission['status'] })}
                >
                  <SelectTrigger className="bg-input border-border text-foreground">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="flex flex-col gap-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setForm({ ...form, approved_id: v === 'none' ? null : parseInt(v) })
                  }
                >
                  <SelectTrigger className="bg-input border-border text-foreground">
            ...
            <TableBody>
              {filtered.map((mission) => (
                <TableRow key={mission.id} className="border-border">
                  ...
                  <TableCell
                    className="text-card-foreground text-sm max-w-[200px] truncate"
                    title={mission.purpose}
                  >
                    {mission.purpose}
                  </TableCell>
                  <TableCell className="text-card-foreground text-sm">{mission.location}</TableCell>
                  <TableCell className="text-card-foreground text-sm whitespace-nowrap">
                    {mission.start_date}
                  </TableCell>
                  <TableCell className="text-card-foreground text-sm whitespace-nowrap">
                    {mission.end_date}
                  ...
                  <TableCell>
                    <Badge
                      variant="outline"
                      className={`${statusColor(mission.status)} flex w-fit items-center gap-1`}
                    >
