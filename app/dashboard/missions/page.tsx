  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-2">
          ...
          <Card key={mission.id}>
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                <div>
                  <CardTitle className="text-base">{mission.purpose || "Untitled Mission"}</CardTitle>
                  <CardDescription>{mission.first_name} {mission.last_name} - {mission.department}</CardDescription>
                </div>
            ...
            <CardContent>
              <div className="flex flex-col gap-2">
                ...
                {mission.approver_name && (
                  <p className="text-xs text-muted-foreground mt-1">Approved by: {mission.approver_name}</p>
                )}
                {mission.status === "Pending" && (
                  <div className="flex items-center gap-2 mt-2">
                    <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={() => updateStatus(mission.id, "Approved")}>
                      <Check className="mr-1 h-3.5 w-3.5" /> Approve
                    </Button>
                    <Button size="sm" variant="outline" className="text-destructive border-destructive/30 hover:bg-destructive/10" onClick={() => updateStatus(mission.id, "Rejected")}>
        ...
        {missions?.length === 0 && (
          <div className="col-span-full text-center text-muted-foreground py-12">No missions found</div>
        )}
      </div>
    </div>
  )
