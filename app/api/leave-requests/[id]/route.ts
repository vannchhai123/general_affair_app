...
export async function PUT(
  ...
  try {
    ...
    leaveRequests[index] = {
      ...
      approved_at: status === "Approved" ? new Date().toISOString().split("T")[0] : leaveRequests[index].approved_at,
      approver_name: status === "Approved" && session ? session.full_name : leaveRequests[index].approver_name,
    }
